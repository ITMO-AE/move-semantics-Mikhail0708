...
```
Почему так произошло: для создания объекта c1 вызвался наш коструктор, с помощью оператора new выделяется память для хранения строки. Затем для создания объекта c2 вызвался конструктор копирования по умолчанию, который за нас сгенерировал компилятор, он, конечно, мог бы и догадаться что мы от него хотим, но компилятор не догодался и произошло примерно такое:
```cpp
c2.ptr = c1.prt;
```
...
1. Выделилось два участка памяти, для c1 и с2, затем c1 присвоили c2, т.е. c1 и c2 стали указывать на один и тот же участок в памяти, значение указателя c1.prt потерялось, соответственно освободить память, которая находится по этому адресу стало просто невозможно.
...
Теперь реализуем оператор присваивания перемещений:
```cpp
